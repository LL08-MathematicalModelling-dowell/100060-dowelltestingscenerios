...
from .serializers import StartBroadcastSerializer, TransitionBroadcastSerializer
...
class StartBroadcastView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            cache.set(f'stream_dict{request.user.id}', stream_dict)

            print(f'XXXXXXXX {stream_dict} XXXXXXXX')
...
#             print(f'<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  stream dictionary{stream_dict} >>>>>>>>>>>>>>>')
