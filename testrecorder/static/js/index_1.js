...
async function insertVideoIntoPlaylist(socket) {
  ...
  try {
    ...
    if (responseStatus === 200) {
      ...
      console.log('<<<<<<<<<< video inserted into playlist >>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
      await sendRTMPURL(socket);

      displayUtilities();

      return video_inserted = true;
    } else {
      console.log(`<<<<<<<<<<<<<<<<<   video playlist insert failed ${responseStatus} --- ${json} >>>>>>>>>>>>>>>>>>`)
    ...
    console.log(`<<<<<<<<<<<<<<<<<   video playlist insert failed ::: ${error} >>>>>>>>>>>>>>>>>>`)
